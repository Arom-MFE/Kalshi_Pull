...
3. Run `pytest -q`. At 0.3.1 it reports `365 passed` in about ten seconds. The tests are offline: they use a fake exchange, a temp data directory, and fail if a socket opens. They ignore `KALSHI_DATA_DIR` and `KALSHI_MAX_RPS` from your shell.
...
- `kalshi_io.discovery.lookup_markets(tickers: list[str]) -> dict[str, dict]`. Batch lookup in both tiers, 100 tickers per request; unknown tickers are simply absent. A ticker that holds whitespace or a comma cannot be found through the `tickers=` list form (it splits on commas and returns nothing for a ticker with a space, observed on 2026-09-21), so each such ticker is looked up on its own through `get_market`: the live route, then the historical one, at most two requests per ticker. 12 cataloged tickers are of that kind. `list_markets(tickers=...)` and `list_historical_markets(tickers=...)` are the bare list form and do not do this.
...
- `kalshi_io.tickers.load_tickers(source: str | list[str]) -> list[str]`. No network. Each element may be a path to a `.txt` file (one ticker per line, `#` lines ignored), a path to a `.json` file (its `tickers` key), a series name (any word for which `CAT/{word}_tickers.txt` exists; it expands to that file, whether or not the series is in `SERIES_LIST`), the word `focus` (expands to `CAT/focus_universe.txt`), a market ticker, or several of these separated by spaces, commas or newlines. Returns a sorted, deduped list. A few old tickers hold a space or a comma themselves (`GDP-232022 Q4-T0.0`, `JOBLESS-22JUL23-C250,000`), so every element is stripped and then handled in this order: (1) an existing path is read whole, spaces included, and a line of a `.txt` file or an element of a `.json` file is stripped and is one ticker, never split further; (2) an element that is exactly a known ticker is taken whole, where known means `kalshi_io.candles.is_cataloged`: the committed catalog, tickers registered by the focus universe, and tickers the API resolved earlier in the process, which `validate_tickers` does (the lookup is a dict built once per process, so a list of 5,027 tickers costs no file read per element); (3) only what is left is a typed argument and is split on whitespace and commas, each token then being a file, `focus`, a series name or a ticker, and tokens are never joined back together. It follows that `load_tickers(load_tickers(x)) == load_tickers(x)` whenever the tickers of `x` are known, which holds for every list the driver hands to a puller (it validates first) and for the poller's derived universe (it registers its tickers). Three limits: a ticker with a space or a comma cannot share a quoted string with other tickers (`"A-1, JOBLESS-22JUL23-C250,000"` becomes `A-1`, `JOBLESS-22JUL23-C250` and `000`), so give it a quoted argument of its own or a file; such a ticker that nothing knows yet must come from a file; and an explicit poller universe (`poll_focus --tickers`, `FOCUS_OVERRIDE`) is not registered, so there such a ticker must be cataloged. Raises `FileNotFoundError` for a missing `.txt` or `.json`, and for `focus` when `CAT/focus_universe.txt` does not exist (the committed catalog ships one; a roll without `--dry-run` and without `--no-focus` rewrites it). It does not expand an event ticker: an event name is taken as a market ticker and ends up unknown.
...
- `kalshi_io.metadata.upsert_market_metadata(rows: list[dict], *, keep_existing: list[dict] | None = None) -> dict`. Replaces stored rows of the same `market_ticker`, keeps every other row, then applies `derive_strikes` to the whole merged frame, and writes atomically under the file lock. Every write therefore fills `strike_source` and the derived strikes of every row, also of rows the call did not touch and of a 32-column file written before 0.3.1. Returns `{"path", "rows", "added", "updated", "kept"}`.
- `kalshi_io.metadata.derive_strikes(df: pandas.DataFrame) -> pandas.DataFrame`. Pure and idempotent. Sets `strike_source` to `api` where the API sent a `strike_type`. For a row without a `strike_type` and without a floor or cap strike whose `yes_sub_title` is `Above N` and nothing else (N with an optional minus sign, thousands separators, decimals and trailing percent signs) it sets `strike_type` to `greater`, `floor_strike` to N and `strike_source` to `subtitle`. Nothing else is derived, a value the API sent is never changed, and other wordings (`3.0% or above`, a bare number, `Before Jan 1, 2025`) stay null. The exchange sends no strike fields for 531 finalized threshold markets of the 2026-09-19 catalog; the rule fills all of them and none of the 11 other rows without a `strike_type`.
- `python -m kalshi_io.metadata --rederive` (`kalshi_io.metadata.main(argv: list[str] | None = None) -> int`). Loads the store, applies `derive_strikes`, writes it back through `upsert_market_metadata` and prints the counts: rows, columns before and after, null `strike_type` before and after, `strike_source` by value, derived rows by series. No request; only `DATA/metadata/markets.parquet` is written. Exit 0, or 1 when there is no store (nothing is created). Run it once after upgrading from 0.3.0: until the store has been written by 0.3.1 the schema pass rejects the 32-column file and the five checks that need the metadata are skipped.
...
- `kalshi_io.quality.format_report(report: kalshi_io.quality.Report) -> str`. One line per check, the coverage table, the CSV path. A daily bar is labelled with the day it covers everywhere in the report, the coverage table (`first day`, `last day`) included: a daily bar ends at midnight Eastern time, 04:00Z or 05:00Z, so the label is the date twelve hours before its `ts_ms`. Hourly, minute, trade and book rows keep the UTC date of their instant.
...
Writes: data files through the pullers; `DATA/logs/{log-name}_{yyyymmdd_hhmmss}.log`; `DATA/logs/backfill_summary_{stamp}.json`; `DATA/logs/backfill_failed_{stamp}_{layer}.txt` per layer with failures; `DATA/state/backfill_journal.jsonl`; the quality CSV `DATA/logs/quality_{yyyymmdd}.csv` unless `--no-audit`. It never writes the coverage CSV `audit_{yyyymmdd}.csv`: that one comes from `pull_audit` only. A run without `--tickers` and without `--retry-failed` holds the lock `backfill_full`. The journal records every (ticker, layer) pair pulled completely while the market was `finalized`; a rerun skips such a pair without a request while the files on disk still agree with the journal. A failed ticker is retried once at the end of its layer. A ticker for which a puller returned no outcome at all is such a failure (`no outcome from the puller (it returned results for: ...)`): it gets the retry, the failure list and exit code 1, and result keys the driver did not ask for are logged at ERROR and kept under `unexpected` in the summary JSON. When three tickers in a row ran out of retries the driver waits 1, 2, 4, 8, 16 minutes between attempts and then gives up. The first Ctrl+C or SIGTERM stops after the current ticker; a second one stops at once. The committed catalog is read, never written.
...
pip install -e ".[dev]"
pytest -q                                      # 365 passed at 0.3.1, offline
...
For a cheap first try use a short-lived settled single-market event such as `RECSSNBER-22`. The summary table has one row per layer; `Result: complete` and exit code 0 mean that every ticker of every layer has an outcome: pulled, final and skipped, or listed under `other` (unknown to the API, no open time, gone upstream). A ticker left without an outcome makes the run exit 1. Run the same command again: a settled event costs no request the second time (`final, skipped` in the summary).
...
ladder = duckdb.connect().execute(f"""
    SELECT m.market_ticker, m.strike_type, m.floor_strike, m.cap_strike, m.strike_source, m.status, m.result, m.expiration_value,
...
Reading rules for whoever computes on it: a `greater` strike asks whether the value ends above `floor_strike` (`greater_or_equal`: at or above), a `less` strike whether it ends below `cap_strike` (`less_or_equal`: at or below), a `between` strike both; the market's `rules_primary` text is the binding wording. Take strikes from these columns, never from the ticker text. `strike_source` says where a strike came from: `api` is the number the exchange sent, `subtitle` is a `greater` strike derived from the exchange's display text `yes_sub_title` (`Above 0.4%`) for an old market for which the exchange sends no strike fields; filter on `api` if you want the exchange's numbers only, and check `rules_primary` when a derived strike matters. `close` is the last trade of the period and is null on a day without a trade; `yes_bid_close` and `yes_ask_close` exist on every bar. A bid of 0.0 with an ask of 1.0 is an empty book, not a quote. Only strikes with a bar on the same `ts_ms` are comparable. `mutually_exclusive` means at most one market of the event resolves yes; it does not mean the outcomes are exhaustive. Intraday ladders work the same way from the hourly or minute layer, or from book snapshots.
...
- Never guess an event or a strike from the ticker text. Events come from the catalog or the API, strikes from the metadata store. The store's only derived value is flagged: `strike_source = subtitle` marks a `greater` strike read from `yes_sub_title`; do not add derivations of your own to the store.
...
| `  NOT TRIED {layer} {ticker}` | driver summary, exit 1 | A ticker of a finished run (not interrupted, not stopped by an outage) that was left without an outcome; at most 20 per layer, the rest are in `not_attempted_tickers` of the summary JSON | Run the same command again |
| `  FAILED {layer} {ticker}: no outcome from the puller (it returned results for: ...)` | driver summary, exit 1 | The puller lost the ticker and reported other keys instead | `--retry-failed`; read `unexpected` in the summary JSON |
...
| `backfill`, `pull_all_freq` | 0 complete: every ticker of every layer has an outcome (also `--estimate-only`, and `--retry-failed` with nothing to retry; a typed ticker that nobody knows is listed as unknown and does not change the code); 1 some tickers failed, or a ticker was left without an outcome although the run was neither interrupted nor stopped by an outage; 2 the API stayed down through every wait, and also a usage error such as an unknown layer; 75 another run holds the lock; 130 interrupted |
| `pull_audit` | 0 always |
...
|-|-|-|
| `schema` | Files whose column set, order or types differ from the fixed schema | Must be 0 on a store written by 0.3.0 or later, once the metadata store has been written by 0.3.1 (`python -m kalshi_io.metadata --rederive`); a rejected file is left out of every other check |
...
| `volume_exchange` | Finalized markets whose daily volume sum differs from the exchange's lifetime volume by more than 0.005, and how many of them have no daily file at all (detail `no daily file; exchange lifetime volume V`) | A market without a daily file is a gap in the store: pull it. The others are the exchange's daily aggregation, see the night daylight saving ends under "API limits and quirks" |
...
| `mutex` | Mutually exclusive events on days when every listed market is quoted: days whose mids sum to more than 1.05 and days below 0.95, counted separately (detail `above the band`, `below the band`); incomplete days; flagged events made of threshold strikes (`period` `flag`) | Only a sum above the band speaks against the flag, which does not promise that the outcomes are exhaustive. The band is inclusive and the sum is rounded to six decimals before it is compared, so the count is the same on every run. A flagged event whose markets are all `greater*` or `less*` strikes (two or more) is a ladder: it is left out of the sums and listed; the stored flag is not touched |
...
| `coverage` | Per series and layer: events, markets, files, rows, first and last day | What the store holds. For the daily layer the two dates are the days the first and the last bar cover, not the UTC dates of their `ts_ms` |
...
- The `tickers=` list form cannot name every ticker (observed on 2026-09-21): it answers 200 without the market for a ticker that holds a comma (`JOBLESS-22JUL23-C250,000`) and for one that holds a space (`GDP-232022 Q4-T0.0`, sent as `+` or as `%20`). The single routes serve both once the ticker is URL-quoted in the path (`/historical/markets/GDP-232022%20Q4-T0.0`), and so do the candle routes and the `ticker=` parameter of the trade routes. The client quotes every path segment. 12 cataloged tickers hold a space or a comma and six hold `>`; their data files carry the same names.
- Strike fields are absent on old threshold markets (observed on 2026-09-21): for 531 finalized markets of nine series, from 2021 to early 2025, the market payload has no `strike_type`, `floor_strike` or `cap_strike` on either route, while `yes_sub_title` reads `Above 0.4%`. The store derives the strike from that text and flags it (`strike_source`). Where the exchange does send a strike it is stored as sent, also `4.099999` for a strike displayed as 4.1 (45 markets) and `0.1` on `PCECORE-22NOV-TN0.1`, whose subtitle and ticker say minus 0.1.
- The night daylight saving ends: on the daily bar that ends 04:00Z after that night the exchange's daily aggregation loses trades while open interest still moves. In the daily store of 2026-09-19, 87 finalized markets had a daily volume sum different from the exchange's lifetime volume (net minus 91,086.02 contracts on 295.8 million); 31 of them show it on the bars ending 2024-11-04 04:00Z and 2025-11-03 04:00Z, on bars with volume 0. A re-fetch returns the same rows, so it is the exchange's data; compare the hourly sums of that day or the trade tape when it matters.
...
One row per market, replaced when the market is read again, kept when the API stops returning it. Sorted by series, event, market. 33 columns in this order:
...
| `strike_type` | string | `greater`, `greater_or_equal`, `less`, `less_or_equal`, `between`, `functional`, `custom`, `structured`; null only when neither the API nor the subtitle gives one |
...
| `strike_source` | string | `api` when the API sent the `strike_type`, `subtitle` when `strike_type` and `floor_strike` were derived from `yes_sub_title`, null when there is neither |
...
| `DATA/logs/backfill_summary_{stamp}.json` | `stamp`, `exit_code`, `complete` (true exactly when `exit_code` is 0), `elapsed_sec`, `layers`, `requests`, `http_429`, `retries`, `unknown` (typed tickers that neither the catalog nor the API knows), `reports` (per layer: `layer`, `done`, `skipped_final`, `failed` and `other` as `{ticker: reason}`, `not_attempted` with the tickers behind it in `not_attempted_tickers`, `unexpected` for result keys a puller returned unasked, `rows`, `requests`, `elapsed_sec`), `store` (per kind: `files`, `rows`) |
...
| `DATA/logs/quality_{yyyymmdd}.csv` | `check, series, event, market, period, count, detail`; `period` holds the layer, the ET day, the month, the status, `skipped` or `flag`, depending on the check |
...
- What the pipeline guarantees: what it stores is what the API sent, cast to numbers and nothing else, with one flagged exception: where the exchange sends no strike fields for an old threshold market, `strike_type` and `floor_strike` are read from its `yes_sub_title` and the row says `strike_source = subtitle`; no duplicates on the dedupe key; rows sorted by time; a ticker's candle history contiguous from market open when it was pulled by the driver or a puller without `--since`; files never half-written.
...
This guide is evidence based and dated, not exhaustive. It was checked on one machine, without a key, on 2026-09-18, against version 0.3.0, and the parts that 0.3.1 changed again on 2026-09-21. Some statements will age wrong as the exchange or the code changes, and some may be wrong already.
