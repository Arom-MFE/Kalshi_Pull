...
- The `tickers=` list form cannot name every ticker (observed 2026-09-21). `GET /historical/markets?tickers=JOBLESS-22JUL23-C250,000` answers 200 with no market, because the value is split on its comma; so does `tickers=GDP-232022 Q4-T0.0`, with the space sent as `+` and as `%20`. The single routes find both once the ticker is URL-quoted in the path, `GET /historical/markets/GDP-232022%20Q4-T0.0` and `GET /historical/markets/JOBLESS-22JUL23-C250%2C000`, and so do the candlestick routes and `GET /historical/trades?ticker=`. This repo quotes every path segment, and its batch lookup (`lookup_markets`) sends a ticker that holds whitespace or a comma through the single routes, live then historical, instead of the list form.
...
| Field | Meaning |
|---|---|
| `strike_type` | `greater`, `greater_or_equal`, `less`, `less_or_equal`, `between`, `functional`, `custom`, `structured`. Absent on single-outcome markets, and absent together with `floor_strike` and `cap_strike` on 531 finalized threshold markets of nine series (KXCPI, KXCPICORE, KXCPIYOY, KXFED, KXGDP, KXJOBLESS, KXPAYROLLS, KXPCECORE, KXU3), from 2021 to as late as early 2025; see below |
...
| `yes_sub_title`, `no_sub_title` | Display text of the outcome, for example `Above 0.4%`. `subtitle` (`>0.4%`) is deprecated and not stored |
...
Strike fields the exchange does not send (observed 2026-09-21). `GET /historical/markets/CPI-21AUG-T0.4`, and the list form asked for `CPI-21AUG-T0.4,KXPAYROLLS-26JUN-T-25000`, return the old market without the keys `strike_type`, `floor_strike` and `cap_strike`, while the KX market in the same response carries `strike_type: greater` and `floor_strike: -25000`; the live route answers 404 for the old ticker, and the event payload carries nothing about strikes. What the old payload does carry is `yes_sub_title: "Above 0.4%"`, `subtitle: ">0.4%"` and a rules text. This repo derives the strike from `yes_sub_title` and flags it: a stored row without a strike type and without a floor or cap strike whose `yes_sub_title` is `Above N` and nothing else gets `strike_type = greater`, `floor_strike = N` and `strike_source = subtitle`; every other row has `strike_source = api`, or null when it has no strike type at all. Nothing else is derived and a value the API sent is never changed. The rule was checked against the rows where the exchange does send a strike: it matches 4,004 of the 4,096 `greater` rows of the 2026-09-19 catalog and no row of another strike type, 3,958 of them agree exactly with `floor_strike`, 45 differ by 0.000001 and one differs in sign (see Nuances); the 92 it does not match have other wordings (`3.0% or above`, a bare number), and there it stays silent.

`mutually_exclusive` is a field of the event, not of the market: at most one market of the event resolves yes. It does not say the outcomes are exhaustive. `GET /events?series_ticker=` lists it for every event of a series in one request. The flag can be wrong: `KXCPICORE-25DEC` carries `mutually_exclusive: true` (observed 2026-09-21 on `GET /events/KXCPICORE-25DEC`) and is a ladder of four `greater` strikes, several of which can resolve yes together.
...
- **The night daylight saving ends, the daily aggregation loses trades.** Observed on the daily store of 2026-09-19 (4,206 finalized markets with a daily file, 794.8 million contracts of lifetime volume): for 87 of them the sum of the daily `volume` differs from the lifetime volume the exchange reports, net minus 91,086.02 contracts on the 295.8 million those 87 traded. 37 of the 87 have a bar on which open interest moves by more than the bar's volume, and 31 of those bars are the ones ending 2024-11-04 04:00Z (9 markets, minus 2,719 contracts) and 2025-11-03 04:00Z (22 markets, minus 48,045), each with volume 0: the bar after the night daylight saving ends, which the exchange ends at 04:00Z like a summer bar. On the 2025 bar 3.8 percent of 506 bars carry volume, against 24.1 percent of the 11,601 other bars of that year that cover a Sunday (2024: 27.3 percent of 66 against 31.7 percent of 2,747). A re-fetch returns identical rows, so the loss is in the exchange's daily aggregation. For the 50 markets without such a bar the daily data alone does not locate the difference. To recover the volume of such a day, sum the hourly candles of that day or the trade tape of the market.
...
- **Delisted duplicates.** `/events?series_ticker=KXGDP` omits `KXGDP-27JAN30` while its markets still show on `/markets` (observed). The catalog adds events it sees only on markets. A duplicate can also stay listed: `FEDDECISION-24JAN` (opened 2023-12-13 18:00Z, five markets, lifetime volume 0 and open interest 0, finalized with results) sits beside `FEDDECISION-24JAN31`, the traded event of that meeting, opened 65 minutes later (283,044 contracts); `GET /events/FEDDECISION-24JAN` answers 200 (observed 2026-09-21). The catalog lists what the exchange lists, and a market without a single candle is complete with no file.
- **Tickers with a space, a comma or `>`.** 12 tickers of the 2026-09-19 catalog hold a space or a comma (ten `GDP-232022 Q4-T*`, `JOBLESS-22JUL23-C250,000`, `JOBLESS-22SEP10-C220,000`) and six hold `>` (`FEDDECISION-23JUN-C>25` and siblings). They are served like any other once the ticker is URL-quoted in the path; only the `tickers=` list form cannot name them (see Live and historical). Observed 2026-09-21: the daily candles of `GDP-232022 Q4-T0.0` (52 candles, volume 7,093) and of `JOBLESS-22JUL23-C250,000` (8 candles, volume 1,153) both sum to the exchange's lifetime volume.
- **Oddities in old strike data.** The exchange stores `floor_strike` `4.099999` for a strike it displays as 4.1 on 45 markets (26 KXU3, 14 KXCPICOREYOY, 5 KXCPIYOY). `PCECORE-22NOV-TN0.1` has `floor_strike` 0.1 while its subtitle (`Above -0.1%`) and its ticker (`TN0.1`) say minus 0.1. 13 old markets have an unfilled rules template (`more than || percent ||%`, `is above % in April 2022`) or a rules text that names another strike (`JOBLESS-21SEP11-C300` and `-C310` both name 320,000). This repo stores all of it as sent.
